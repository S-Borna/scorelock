                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {standings.map((s, i) => (
                            <tr
                                key={s.team.id}
                                className={`border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors ${i < 4 ? "border-l-2 border-l-scorelock-600" : ""
                                    }`}
                            >
    ...
    return (
        <div className="flex gap-1 justify-center">
            {form.split("").slice(-5).map((r, i) => (
                <span
                    key={i}
                    className={`w-5 h-5 rounded text-[10px] flex items-center justify-center font-bold ${colors[r] || "bg-gray-700"
                        }`}
                >
                    {r}
                </span>
